...
module.exports = function (app, hexo) {
  ...
  app.use(hexo.config.root + 'bridge', auth(new authStrategy(hexo)));
  app.use(hexo.config.root + 'bridge/login', function (req, res) {
      if (req.method === 'POST') {
          req.authenticate(['adminAuth'], function(error, done) {
              if (done) {
                  res.writeHead(302, { 'Location':  hexo.config.root + "bridge/" });
                  res.end();
              }
          });
      } else {
  ...
  app.use(hexo.config.root + 'bridge/', function (req, res, next) {
